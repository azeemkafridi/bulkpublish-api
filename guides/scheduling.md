...
- **"Publish now" with approval** — someone who needs approval publishes now by scheduling the post for now with `requestApproval` (or a contributor key) and `"publishWhenApproved": true`. The post then goes out as soon as it is approved, however late that is. `publishWhenApproved` (boolean, default `false`) is stored only while the post is `pending` (approving or rejecting clears it, and a post entering review again starts from `false`), and every post response includes it. On `PUT`, an explicit value wins; omitted, it is kept, except that sending a different `scheduledAt` clears it.
- **The approval queue** — `GET /api/posts?approvalStatus=pending`.
- **Approving** — `POST /api/posts/:id/approve` (requires a role with `post:approve`: owner, admin, approver). Releases the post: it publishes at its scheduled time, or immediately if that time passed less than 15 minutes ago. If the scheduled time passed more than 15 minutes ago, the optional body `{ "whenLate": "publish" | "hold" }` decides: `"publish"` publishes it immediately (`status` `"publishing"`); `"hold"` approves it but does not publish it: it comes back with `status` `"draft"` (`approvalStatus` `"approved"`, `scheduledAt` unchanged) and the author is notified to choose a new time. Without `whenLate`, the post's `publishWhenApproved` decides (`true` = publish, `false` = hold). `whenLate` has no effect when the time is still ahead or passed less than 15 minutes ago; any other value returns `400`. Publishing a pending/rejected post as an approver implicitly approves it.
...
Both endpoints also accept an optional `ifUnmodifiedSince`: the post's `updatedAt` as you reviewed it. When sent, the call only succeeds on that exact version, so an edit made after you loaded the post is never approved (and published) unseen. Both set `publishWhenApproved` to `false`.

Both endpoints return the post on 200, `400` if the post is not awaiting approval, `ifUnmodifiedSince` is not a timestamp, or (on approve) `whenLate` is not `publish`/`hold`, `403` if the role lacks `post:approve`, `404` if not found, and `409` if the post changed since you loaded it (checked when `ifUnmodifiedSince` is sent; `error.updatedAt` carries the current value) or is no longer awaiting approval — reload it and review again.
